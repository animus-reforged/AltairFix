...
static EngineType DetectEngine()
{
    ...
    __try {
        if (MEMCMP32(0x00401375 + 1, 0x42d6)) return ENGINE_DX9;
        else if (MEMCMP32(0x004013DE + 1, 0x428d)) return ENGINE_DX10;
    }
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        if (g_config.windowMode != 0)
        {
            ...
            switch (engine)
            {
            case ENGINE_DX9:  Logger::Get()->info("Engine=DX9");  WindowedMode::HookD3D9();  break;
            case ENGINE_DX10: Logger::Get()->info("Engine=DX10"); WindowedMode::HookD3D10(); break;
            default:          Logger::Get()->warn("Engine=Unknown"); break;
            }
