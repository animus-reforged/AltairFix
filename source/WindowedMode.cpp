...
// =================== Helpers ===================
HWND WindowedMode::GetGameWindow() { return s_gameWindow; }
...
BOOL WINAPI WindowedMode::HookedGetMessage(LPMSG lpMsg, HWND hWnd, UINT min, UINT max)
{
    ...
    if (result && lpMsg) FilterMessage(lpMsg);
    return result;
}
BOOL WINAPI WindowedMode::HookedPeekMessage(LPMSG lpMsg, HWND hWnd, UINT min, UINT max, UINT remove)
{
    ...
    if (result && lpMsg && (remove & PM_REMOVE)) FilterMessage(lpMsg);
    return result;
}
...
void WindowedMode::AdjustPresentParameters(D3DPRESENT_PARAMETERS* params)
{
    ...
    if (g_config.windowMode == 1) { // Borderless
        ...
        params->Windowed = TRUE;
        params->FullScreen_RefreshRateInHz = 0;
        if (g_config.windowWidth > 0)  params->BackBufferWidth = g_config.windowWidth;
        if (g_config.windowHeight > 0) params->BackBufferHeight = g_config.windowHeight;
...
LRESULT CALLBACK WindowedMode::WindowProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (msg)
    {
    case WM_ACTIVATE: if (LOWORD(wParam) != WA_INACTIVE) ConfineCursorToWindow(hWnd); else ClipCursor(nullptr); break;
    case WM_KILLFOCUS: ClipCursor(nullptr); break;
    case WM_SETFOCUS:  ConfineCursorToWindow(hWnd); break;
    case WM_SETCURSOR: if (GetForegroundWindow() == hWnd) ConfineCursorToWindow(hWnd); break;
    }
...
HRESULT WINAPI WindowedMode::HookedCreateDevice(IDirect3D9* self, UINT adapter, D3DDEVTYPE type, HWND win, DWORD flags, D3DPRESENT_PARAMETERS* p, IDirect3DDevice9** outDev)
{
    ...
    if (SUCCEEDED(hr) && outDev && *outDev)
    {
        ...
        if (!s_TrueReset) { MH_CreateHook(s_pDeviceVTable[16], &HookedReset, (void**)&s_TrueReset); MH_EnableHook(s_pDeviceVTable[16]); Logger::Get()->debug("DX9 Reset hook installed"); }
        if (!s_TruePresent) { MH_CreateHook(s_pDeviceVTable[17], &HookedPresent, (void**)&s_TruePresent); MH_EnableHook(s_pDeviceVTable[17]); Logger::Get()->debug("DX9 Present hook installed"); }
    }
    return hr;
}
HRESULT WINAPI WindowedMode::HookedReset(IDirect3DDevice9* self, D3DPRESENT_PARAMETERS* params)
{
    ...
    AdjustPresentParameters(params); return s_TrueReset(self, params);
}
...
void WindowedMode::HookD3D9()
{
    ...
    if (!d3d) { Logger::Get()->error("Failed to Direct3DCreate9"); return; }
...
void WindowedMode::HookD3D10()
{
    ...
    if (!hD3D10) hD3D10 = LoadLibraryW(L"d3d10.dll");
    if (hD3D10)
    {
...
{
    if (MH_Initialize() != MH_OK) { Logger::Get()->error("MH_Initialize failed"); return; }
    HookD3D9();
    HookD3D10();
    HookFocusFunctions();
